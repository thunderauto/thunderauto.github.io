...
const Navbar = () => {
  ...
  return (
    <div className="sticky top-0 left-0 z-[60] w-full bg-color1 font-MontF border-b-2">
      <div className="relative rounded-xl flex justify-between items-center w-full max-w-7xl mx-auto py-5 px-5 font-Mont">
        <a
          href="#"
          className="cursor-pointer hover:opacity-80 transition-opacity duration-300"
        ...
        {/* Mobile Menu */}
        {toggle && (
          <ul className="md:hidden bg-color1 w-full absolute top-full left-0 items-start animate-flip-down duration-200 flex flex-col text-color3 space-y-6 sm:space-y-8 px-10 py-5 sm:px-20 cursor-pointer z-[70] shadow-lg">
            {links.map((link, index) => (
              <NavLink
                className={({ isActive }) =>
                  isActive
